...
class  ZbProx implements ZbLink.ConnectListener
{
	...
	public int syncConnect(String host, int port) 
	{
		...
		synchronized(mRequestLock) {
			if (mConnect != CONNECT_OFF) return -1;
			if (mRequestLock.reqToken != 0) return -2;
	...
	public int syncDisConnect() 
	{
		int r;
		synchronized(mRequestLock) {
			if (mConnect != CONNECT_ON) return 0;
	...
	private byte[] wait_data()
	{
		...
		synchronized(mRequestLock) {
			...
			try{
				...
				//e.printStackTrace();
				Log.d(TAG, "**************wait response data timeout................");
				mRequestLock.reqToken = 0;
				return null;
			}
	...
	byte[] syncRequestSYS_GET_DEVICE_INFO()
	{
		...
		synchronized(mRequestLock) {
			...
			ret = mRequestLock.request(SYS_GET_DEVICE_INFO, SYS_GET_DEVICE_INFO_RESPONSE, 0, null);
			if (ret != 0) {
	...
	byte[] syncRequestSYS_APP_MSG(int ep, byte[] dat)
	{
		...
		if (mRequestLock.reqToken != 0) 
			...
			data[0] = (byte) (ep&0xff);
			for (int i=0; i<dat.length; i++) data[i+1] = dat[i];
	...
	byte[] syncRequestZDO_IEEEADDRESS(int naddr, int rty, int idx)
	{
		...
		req[3] = (byte) idx;
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken != 0) return null;
			Log.d(TAG, "syncRequestZDO_IEEEADDRESS");
			ret = mRequestLock.request(ZDO_IEEEADDRESS, ZDO_IEEEADDRESS_RESPONSE, 
					ZDO_IEEEADDRESS_RESPONSECB, req);
			if (ret != 0){
	...
	byte[] syncRequestZDO_NODEDESCRIPTOR(int dstaddr, int intaddr, int sec)
	{
		...
		synchronized(mRequestLock) {
			...
			ret = mRequestLock.request(ZDO_NODEDESCRIPTOR, ZDO_NODEDESCRIPTOR_RESPONSE,
					ZDO_NODEDESCRIPTOR_RESPONSECB, req);
			if (ret != 0){
	...
	**************************************************************************/
	...
	/*
	public int requestSYS_RESET()
	{
		byte[] b = new byte[1];
		b[0] = 0;
		return mLink.requestSendData(makeRequest(0x0005, b));
	}
	private void doresponseSYS_RESET(byte[] b)
	{
		
	}
	
	public int requestSYS_PING()
	{
		return mLink.requestSendData(makeRequest(0x0007));
	}
	...
	public int requestSYS_VERSION()
	{
		return mLink.requestSendData(makeRequest(0x0008));
	}
	private void doresponseSYS_VERSION(byte[] b)
	{
		Log.d(TAG, "sys version:"+ b.toString());
	}
	
	
	public int requestSYS_GET_DEVICE_INFO()
	{
		return mLink.requestSendData(makeRequest(0x0014));
	}
	*/
...
/*	private void doresponseSYS_GET_DEVICE_INFO(byte b[])
	{
		Log.d(TAG, "do get sys device info.");
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken == SYS_GET_DEVICE_INFO_RESPONSE) {
				mRequestLock.dat = b;
				mRequestLock.reqToken = 0;
				mRequestLock.notify();
			}
...
*/
	...
	**************************************************************************/
	...
	{
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken == ZDO_IEEEADDRESS_RESPONSE) {
				if (b[0] == 0) {
	...
	private void doZDO_IEEEADDRESS_RESPPONSECB(byte[] b)
	{
		Log.d(TAG, "doZDO_IEEEADDRESS_RESPPONSECB....");
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken == ZDO_IEEEADDRESS_RESPONSECB) {
				mRequestLock.reqToken = 0;
				mRequestLock.dat = b;
				mRequestLock.notify();
	...
	private void doZDO_NODEDESCRIPTOR_RESPONSE(byte[] b)
	{
		Log.d(TAG, "doZDO_NODEDESCRIPTOR_RESPONSE....");
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken == ZDO_NODEDESCRIPTOR_RESPONSE) {
				if (b[0] == 0) {
	...
	private void doZDO_NODEDESCRIPTOR_RESPONSECB(byte[] b)
	{
		Log.d(TAG, "doZDO_NODEDESCRIPTOR_RESPONSECB....");
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken == ZDO_NODEDESCRIPTOR_RESPONSECB) {
				mRequestLock.reqToken = 0;
				mRequestLock.dat = b;
				mRequestLock.notify();
			}
...
*/
	public void onConnectData(byte[] b) {
		...
		Log.d(TAG, "RECV:"+Tool.byte2string(b));
		...
		synchronized(mRequestLock) {
			if (mRequestLock.reqToken != 0 &&
				...
				if (mRequestLock.reqRespcb != 0 && mRequestLock.reqResp == res) {//&& res == mRequestLock.reqRespcb) {
		...
		case SYS_RESET_RESPONSE:
			doresponseSYS_RESET(dat);
			break;
		case SYS_PING_RESPONSE:
			doresponseSYS_PING(dat);
			break;
		case SYS_VERSION_RESPONSE:
			doresponseSYS_VERSION(dat);
			break;
		case SYS_GET_DEVICE_INFO_RESPONSE:
			doresponseSYS_GET_DEVICE_INFO(dat);
			break;
		
		case ZDO_IEEEADDRESS_RESPONSE:
			doresponseZDO_IEEEADDRESS(dat);
			break;
		case ZDO_IEEEADDRESS_RESPONSECB:
			doZDO_IEEEADDRESS_RESPPONSECB(dat);
			break;
		case ZDO_NODEDESCRIPTOR_RESPONSE:
			doZDO_NODEDESCRIPTOR_RESPONSE(dat);
			break;
		case ZDO_NODEDESCRIPTOR_RESPONSECB:
			doZDO_NODEDESCRIPTOR_RESPONSECB(dat);
			break;
			
		default:
			Log.d(TAG, String.format("unknow response:%04X", res));
			break;
		}
		*/
	}
